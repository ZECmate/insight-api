...
StatusController.prototype.circulation = function (req, res) {
  var self = this;
  const height = self.node.services.bitcoind.height;

  // // Mining slow start
  // // The subsidy is ramped up linearly, skipping the middle payout of
  // // MAX_SUBSIDY/2 to keep the monetary curve consistent with no slow start.
  // if (height < 10000) {
  //   var subsidy = 12.5 * (height - 1) / 20000
  // } else if (height < 20000) {
  //   var subsidy = 12.5 * height / 20000
  ...
  // var halvings = Math.floor((height - 20000) / 840000);
  ...
  var sum = 125000;
  if (height >= 20000) {
    sum = sum + ((height - 19999) * 12.5)
    if (height >= 653600) {
      sum = sum - ((height - 653599) * 12.5)
    }
  }
  if (height >= 653600 && height < 840000) {
    sum = sum + ((height - 653599) * 6.25)
